...
def erase_beach_noise(arr, waterMask):
    ...
    print(
        f"Removing blocks of +{TOO_BIG_TO_BE_A_SHIP} pixels between {MIN_NOISE_BRIGHTNESS} and {MAX_NOISE_BRIGHTNESS}"
    )
    ...
    histogram, _ = numpy.histogram(
        labels, bins=num_labels, range=(0.5, num_labels + 0.5)
    )
    ...
    for ii in labels_pass:
        ...
        if chunk_size > TOO_BIG_TO_BE_A_SHIP:
            print(
                f"Removing noise chunk {ii} {chunk_count}/{label_count}. Size: {chunk_size}"
            )
...
def basic_is_bright(dataset_01, dataset_02, waterMask):
    ...
    return isBright, int(numpy.minimum(x_size, y_size) / 50.0), geomatrix, proj
...
def big_is_bright(dataset_01, dataset_02, waterMask):
    ...
    print(
        f"bright_cutoff: {max2} - ({mean1} + {mean2}) - {pixel_range} = {bright_cutoff}"
    )
...
def group_pixels(labels, labels_pass, dataset=None):
    ...
    for label in labels_pass:  # [::100]: #[::1000]:
        ...
        print(
            f"Grouping label pass {label} ({label_counter}/{label_count}). Size: {ship_size}"
        )
    ...
    print(
        f"Found {len(loc_labels_list)} Label areas with BRIGHTNESS > {BRIGHT_THRESHOLD}"
    )
...
def get_rectangle(c_center, r_center, rectLen):
    xy_array = (int(c_center - rectLen / 2.0), int(r_center - rectLen / 2.0))
...
def ship_low_mem(scene_1, scene_2, output_file_name=None):
    ...
    water_mask = get_geotiff_watermask(
        "coop-static-input", "water_masks/ak_watermask.tiff", cut_poly, scene_2
    )
    ...
    is_bright, rect_len, geomatrix, proj = basic_is_bright(
        early_scene_data_path, later_scene_data_path, water_mask
    )
