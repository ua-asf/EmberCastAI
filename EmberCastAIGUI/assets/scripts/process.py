...
files = generate_geocoded_grd(
    results[0].properties["sceneName"], out_dir=f"{file_path}/data"
)
...
sar_files = [
    ...
    if file.endswith(".tiff") and file != "merged.tiff" and file != "merged_wkt.tiff"
]
...
draw_wkt_to_geotiff(
    polygons, input_file, output_file=f"{file_path}/data/merged_wkt.tiff"
)
