        ...
        if kmz_file.endswith(".kmz"):
            ...
            for placemark in root.findall(".//kml:Placemark", namespace):
                ...
                if (
                    ...
                    and "heat perimeter" in name.text.lower()
                ):
                    polygon_elements = placemark.findall(
                        ...
                        if polygon.text:
                            ...
                            for triple in raw_coordinates:
                                try:
                                    ...
                                    print(
                                        f"Skipping invalid coordinate triple: {triple} ({e})"
                                    )
            ...
            if not polygons:
                print(
                    f"No valid 'Heat Perimeter' polygons found in KMZ file: {kmz_path}. Skipping."
                )
                shutil.rmtree(extracted_dir, ignore_errors=True)
                continue

            # Now that we have polygons, create directories
            output_geotiff_dir = ensure_output_dir_structure(
                kmz_dir, os.path.join(output_dir, "geotiff"), relative_dir
            )
            output_png_dir = ensure_output_dir_structure(
                kmz_dir, os.path.join(output_dir, "png"), relative_dir
            )
            output_wkt_dir = ensure_output_dir_structure(
                kmz_dir, os.path.join(output_dir, "coordinates"), relative_dir
            )
