...
def execute_cmr_search(url):
    ...
    while True:
        ...
        print(f"CMR query returned {r.status_code} {r.reason}")
        print(f"CMR query URL: {r.url}")
...
def get_relevant_metadata(scene_metadata):
    ...
    geometry = scene_metadata["SpatialExtent"]["HorizontalSpatialDomain"]["Geometry"][
        "GPolygons"
    ][0]["Boundary"]["Points"]
    ...
    browse = next(
        (item["URL"] for item in urls if item["Type"] == "GET RELATED VISUALIZATION"),
        None,
    )
