...
WKT = (
    'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["unnamed",6378137,298.25722356049,'
...
def overwrite_tiff(tif_file, output, data_file):
    ...
    print(
        f"Data was stetched {y_size} -> {new_width}, scaling factor: {y_scale_factor:0.4f}"
    )
    ...
    print(
        f"Original data was {y_size} x {x_size} ({original_data_size}b), in data is {len(data)}"
    )
    ...
    for gcp in original_gcp:
        ...
        log.debug(
            f"Shifting (x{gcp.GCPX},y{gcp.GCPY},z{gcp.GCPZ}) from ({gcp.GCPLine},{gcp.GCPPixel}) to ({gcp.GCPLine},{new_y_value})"
        )
